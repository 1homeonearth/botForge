...
- Runtime feature modules are isolated in per-feature folders (`features/squire/`, `features/bard/`) so feature bundles are easy to pull in, remove, or swap across bots; they implement deterministic migration behaviors (autoban, embed builder, XP/levels, moderation action records, rainbow bridge relay ledger, setup summary helpers, and Bard logging/moderation/starboard/welcome payload shaping routed through runtime intents).
